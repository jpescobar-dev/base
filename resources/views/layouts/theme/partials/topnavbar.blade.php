<div class="topbar-nav header navbar mb-3" role="banner">
    <nav id="topbar" class="w-100">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">

            <ul class="list-unstyled menu-categories mb-0" id="topAccordion">
            ...
            <div class="topbar-breadcrumbs d-flex align-items-center flex-wrap gap-2">
                @foreach(($breadcrumbs ?? []) as $index => $crumb)
                    ...
                    @if(!empty($crumb['url']) && $index !== count($breadcrumbs) - 1)
                        <a href="{{ $crumb['url'] }}" class="text-muted small">
                            {{ $crumb['label'] }}
                        </a>
                    @else
                        <span class="small font-weight-semibold">{{ $crumb['label'] }}</span>
                    @endif
                @endforeach
            </div>
        </div>
